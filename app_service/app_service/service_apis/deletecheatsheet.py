...
from app_service.service_api_handlers import \
	 post_delete_cheatsheet_handler
from app_service.utils.resource import Resource
from app_service.utils.auth import get_user

class DeleteCheatSheet(Resource):
	""" 
	This class deletes a cheatsheet
	"""

	def post(self):
		try:
			data = request.get_json(force=True)
			app.logger.debug("Call to delete cheatsheet:"+str(data))

			return post_delete_cheatsheet_handler.handle_request(data)        
